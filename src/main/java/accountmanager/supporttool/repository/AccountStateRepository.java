...
public class AccountStateRepository {
    ...
    public void fixAccountManagerStudentState(AccountStateDTO accountStateDTO){
        ...
        if(accountStateDTO.isHasStateRecord()){
            deleteUserAccountState(accountStateDTO.getValue());
        }
        //call sp
        List<AMTriggerSPCallInfo> amTriggerSPCallInfoList = prepareAMTriggerSPCall(accountStateDTO.getStudentNumber());
        int enrollmentId = amTriggerSPCallInfoList.get(0).getEnrollment_Id();
        if(enrollmentId != 0){
            executeAMTriggerProcedure(enrollmentId);
        }
    }
